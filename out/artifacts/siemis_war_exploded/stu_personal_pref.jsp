        ...
        <div class="unit-information lastpart">
	        ...
	        <form>
		        <table id="addTable">
				    ...
				    <%
					    List<StuPref> list = (List<StuPref>)request.getAttribute("stuPreflist");
					    if(list != null) {
						    for(int i = 0; i < list.size(); i++) {
							    StuPref stu_pref = list.get(i);
				    %>
		            <tbody>
		                <tr>
		                    <td><label style="margin-left: 100px;">偏好类型：</label></td>
		                    <td><input type="text" id="pref_type" value="<%=stu_pref.getPref_type() %>"/></td>
		                    <td>项目规模：</td>
		                    <td><input type="text" id="pref_name" value="<%=stu_pref.getPref_name() %>"/></td>
		                    <td><input type="button" value="删除" onclick="deleteRow(this)" /></td>
		                </tr>
		            </tbody>
	           	    <%		
