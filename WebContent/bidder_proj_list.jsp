...
<%@page import="beans.BidderApply" %>
<%@page import="dao.BidderApplyDao"%>
...
<%
	...
	String bidder_num = (String)request.getAttribute("bidder_num");
%>
<head>
    <meta charset="UTF-8">
            ...
            <div class="unit-list clearfix">
	           <form action = "CurProjQuery" method = "post">
	               ...
	               <div class="querycol"><label>项目规模</label> : <input type="text" value="" name="proj_country" id="proj_country"/></div>
	                <input type="hidden" id="bidder_num" name="bidder_num" value="<%=bidder_num%>">
                    ...
                    <tr>
                        ...
                        <td><%=indexproj_info.getProj_apply_ddl() %></td>
                       	...
                       	<%
                       	    	...
                       	    	if(bidder_apply.getApply_status().equals("等待审批")) {
                       	%>
                       	<td>等待审批<a href="javascript:void(0);" onclick="delApplyInfo(<%=indexproj_info.getProj_no()%>)">撤销</a></td>
                       	<td><%=bidder_apply.getAprv_tender() %> </td>
                       	<%
                       	    	} else if(bidder_apply.getApply_status().equals("通过")) {
                       	%>
                       	<td>审核通过</td>
                       	<td><%=bidder_apply.getAprv_tender() %></td>
                       	<%  	
